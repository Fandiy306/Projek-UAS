...
if(isset($_POST['order_btn'])){
   ...
   $price_total = 0;
   $product_name = []; // Inisialisasi array di luar loop
   
   if(mysqli_num_rows($cart_query) > 0){
      while($product_item = mysqli_fetch_assoc($cart_query)){
         $product_name[] = $product_item['name'] .' ('. $product_item['quantity'] .') ';
   
         // Hitung harga produk tanpa memformatnya
   ...
   // Format total harga di sini jika diperlukan
   $formatted_price_total = number_format($price_total);
   
   // Contoh tampilan total harga
   echo "Total Harga: $formatted_price_total";
